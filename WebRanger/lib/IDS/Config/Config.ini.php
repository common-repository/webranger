...
; WebRanger Config.ini

; General configuration settings

[General]

    ; basic settings - customize to make the WebRanger work at all
    filter_type     = xml
    ...
    ; By default, those files are used that are being shipped with WebRanger
    HTML_Purifier_Cache = vendors/htmlpurifier/HTMLPurifier/DefinitionCache/Serializer

    ; define which fields contain html and need preparation before
    ; hitting the WebRanger rules (new in WebRanger 0.5)
    ;html[]          = POST.__wysiwyg

    ; define which fields contain JSON data and should be treated as such
    ; for fewer false positives (new in WebRanger 0.5.3)
    ;json[]          = POST.__jsondata

    ; define which fields shouldn't be monitored (a[b]=c should be referenced via a.b)
    exceptions[]    = GET.__utmz
    exceptions[]    = GET.__utmc
    exceptions[]    = HEADER.Content-Type
    exceptions[]    = HEADER.Cookie
    exceptions[]    = /COOKIE\.mp/
...
[Caching]
    ...
    ; database cache
    wrapper         = "mysql:host=localhost;port=3306;dbname=WebRanger"
    user            = WebRanger_user
    password        = 123456
    table           = cache
